...
3. **Clean each record.** `clean_record()` drops Socrata internal fields (`:id`, `_submission_details`,
   `__computed_region_*`, georeferenced point columns), lowercases and normalises field names for
...
|---|---|---|---|
| `BQ_PROJECT_ID` | yes | — | Target GCP project |
...
Worth stating plainly, because both are deliberate tradeoffs rather than oversights.
